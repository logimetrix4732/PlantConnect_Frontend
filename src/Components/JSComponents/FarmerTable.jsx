...
export default function FarmerTable({
  ...
  useEffect(() => {
    ...
    const filtered = data?.filter(
      (item) =>
        ...
        item?.FarmerCode?.toLowerCase()?.includes(search?.toLowerCase())
    );
  ...
  const calculateTotals = (data) => {
    return data.reduce(
      (totals, row) => {
        ...
        totals.Estimated_Prod += Number(row.Estimated_Prod) || 0;
        return totals;
      },
      {
        landArea: 0,
  ...
  return (
    <React.Fragment>
      {/* <Card className="HeadingCard">Farmer detailed report</Card> */}
      <Card
        style={{
          padding: hideSpComponents ? "12px 12px 12px 12px" : "",
          borderRadius: "12px",
        }}
        elevation={6}
      >
                      ...
                      <StyledTableRow key={index}>
                        ...
                        >
                          <Tooltip
                            ...
                            onClick={() => handleClickFarmerOpen(row.id)}
                          >
                            <PersonIcon />
                          </Tooltip>
                        </StyledTableCell>
